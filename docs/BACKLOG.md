...
| B-021.5 | Stage 12: Slack Minimal Loop Bridge（`slack_bridge.py`：`parse_devos_command`（DEVOS_BRIDGE_PREFIX 前缀过滤，空 instruction 忽略）+ `build_slack_thread_id`（thread_ts > ts 优先）+ `build_devos_start_payload`（DEVOS_DEFAULT_REPO_PATH 可选）+ `call_devos_start`（POST /devos/start，trust_env=False）+ `handle_slack_event`（bot/subtype 忽略，dry-run 模式，后端错误安全回复）；DEVOS_BRIDGE_DRY_RUN=true 不调用后端；不调用 apply/test/commit；`test_slack_bridge.py`：10 scenarios A–J（non-devos ignored, extract instruction, thread_ts priority, ts fallback, bot/subtype ignored, dry-run no backend call, backend payload verify, backend error safe reply, repo path included, empty instruction ignored）；`scripts/run_slack_bridge_mock.sh`：mock event runner + dry-run 默认 + backend health check；`.env.example` 新增 DEVOS_BRIDGE_PREFIX / DEVOS_DEFAULT_REPO_PATH / DEVOS_BRIDGE_DRY_RUN；README 新增 B-021.5 章节；CI 不调用真实 Slack；无 token 泄漏；139 Java + 64+ Python tests 全绿） | commit `feat(stage12): B-021.5` — 2026-05-02 |
---
